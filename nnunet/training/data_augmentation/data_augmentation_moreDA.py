...
def get_moreDA_augmentation(dataloader_train, dataloader_val, patch_size, params=default_3D_augmentation_params,
    ...
    if use_nondetMultiThreadedAugmenter:
        ...
        batchgenerator_train = NonDetMultiThreadedAugmenter(dataloader_train, tr_transforms, params.get('num_threads')*2,
                                                            params.get("num_cached_per_thread")*10, seeds=seeds_train,
                                                            pin_memory=pin_memory)
    else:
        batchgenerator_train = MultiThreadedAugmenter(dataloader_train, tr_transforms, params.get('num_threads')*2,
                                                      params.get("num_cached_per_thread")*10,
    ...
    if use_nondetMultiThreadedAugmenter:
        ...
        batchgenerator_val = NonDetMultiThreadedAugmenter(dataloader_val, val_transforms,
                                                          max(params.get('num_threads')*2 // 2, 1),
                                                          params.get("num_cached_per_thread")*10,
