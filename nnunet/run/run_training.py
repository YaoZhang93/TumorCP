...
def main():
    ...
    parser.add_argument("-run", type=int, required=False, default=None,
                        help='0, 1, 2, 3, ... for stability check, 3 runs or 5 runs are recommended')
    ####### @JW 2021/03/08 ########
    ...
    if args.run is not None:
        run = int(args.run)
        output_folder_name = os.path.join(output_folder_name, f'run_{run}')
    max_num_epochs = 2 if args.dev else args.epoch
    num_batches_per_epoch = 2 if args.dev else 250
    num_val_batches_per_epoch = 2 if args.dev else 50
    epoch_batch = dict(
    ...
    trainer = trainer_class(plans_file, fold, output_folder=output_folder_name, dataset_directory=dataset_directory,
                            ...
                            # Added keyword 'epoch_batch' @JW 20201/03/08
                            **epoch_batch)
