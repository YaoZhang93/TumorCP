...
if __name__ == "__main__":
    ...
    train_id_range = range(0, 131)
    test_id_range = range(131, 131)
    train_dir = "/dfsdata2/zhongcheng_data/zhangyao_data/DB/lits17/ori_data/"
    test_dir = "/dfsdata2/zhongcheng_data/zhangyao_data/DB/lits17/ori_data/"

    output_folder = "../../data/nnUNet_raw_data/Task100_LiTSbaseline/"
    ...
    nii_files_tr_data = [x for x in np.sort(glob.glob(train_dir+'data_volume/*.nii.gz')) if int(x.split('/')[-1].split('.')[0].split('_')[1]) in train_id_range]
    print('train data:', len(nii_files_tr_data))
    nii_files_tr_seg = [x for x in np.sort(glob.glob(train_dir+'seg_volume/*.nii.gz')) if int(x.split('/')[-1].split('.')[0].split('_')[1]) in train_id_range]
    print('train label:', len(nii_files_tr_seg))
    nii_files_ts = [x for x in np.sort(glob.glob(train_dir+'data_volume/*.nii.gz')) if int(x.split('/')[-1].split('.')[0].split('_')[1]) in test_id_range]
